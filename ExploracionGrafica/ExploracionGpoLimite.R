...
#Identificadores del limite
ids <- read.csv("../ConjuntosDatos/casos_limite.csv", stringsAsFactors = TRUE)
...
path <- "./Graficas/LIMITE/"
#DOMINIO Grupo Codigo
limite_viv <- endutih_vivhogar %>% filter(paste0(DOMINIO,Grupo, ENT) %in% paste0(ids$DOMINIO, ids$Grupo, ids$Codigo))
limite_viv$Grupo<- paste("Grupo", limite_viv$Grupo)
limite_viv$ENT <-sapply(limite_viv$ENT, function(x){gsub(x, estados[estados$Codigo==x, 3], x)})

limite_res <- endutih_res %>% filter(paste0(DOMINIO,Grupo, ENT) %in% paste0(ids$DOMINIO, ids$Grupo, ids$Codigo))
limite_res$Grupo<- paste("Grupo", limite_res$Grupo)
limite_res$ENT <-sapply(limite_res$ENT, function(x){gsub(x, estados[estados$Codigo==x, 3], x)})

limite_usu <- endutih_usu %>% filter(paste0(DOMINIO,Grupo, ENT) %in% paste0(ids$DOMINIO, ids$Grupo, ids$Codigo))
limite_usu$Grupo<- paste("Grupo", limite_usu$Grupo)
limite_usu$ENT <-sapply(limite_usu$ENT, function(x){gsub(x, estados[estados$Codigo==x, 3], x)})

limite_usu2 <- endutih_usu2 %>% filter(paste0(DOMINIO,Grupo, ENT) %in% paste0(ids$DOMINIO, ids$Grupo, ids$Codigo))
limite_usu2$Grupo<- paste("Grupo", limite_usu2$Grupo)
limite_usu2$ENT <-sapply(limite_usu2$ENT, function(x){gsub(x, estados[estados$Codigo==x, 3], x)})
...
ggplot(limite_viv, mapping = aes(y=as.factor(P1_1),x=FAC_VIV), )+
    geom_bar(aes(x=after_stat(prop)*100, group=factor(0)), color=lmain,fill=main)+
    facet_grid(
        cols = vars(Grupo), rows = vars(ENT), scales = "fixed",
    )+
    labs(
...
ggsave(
    path=path, filename= "LIMITE_material_piso.pdf",
    device="pdf", dpi="retina",
    width=18, height=25, units="cm"
)

# EDAD 1+ 3.322*log10(sum(limite_res$FAC_HOGAR))
ggplot(limite_res, aes(EDAD, weight=FAC_HOGAR))+
    geom_histogram(aes(y=after_stat(density)*(100/25)*100), color=lmain, fill=main, bins = 26)+
    facet_grid(
        cols = vars(Grupo), rows = vars(ENT), scales = "fixed",
    )+
    geom_vline( xintercept =sum(limite_res$FAC_HOGAR*limite_res$EDAD)/sum(limite_res$FAC_HOGAR), color=lmain)+
    labs(
...
ggsave(
    path=path, filename= "LIMITE_hist_edad.pdf",
    device="pdf", dpi="retina",
    width=18, height=25, units="cm"
)
...
ggplot(modalidad_servicio, aes(y=Modalidad, x=FAC_HOG))+
    geom_bar(stat="identity",color=lmain, fill=main)+
    labs(
...
ggsave(
    path=path, filename= "LIMITE_modalidad_compu.pdf",
    device="pdf", dpi="retina",
...
ggplot(limite_res, aes(x = as.factor(ENT), y = FAC_HOGAR, fill = as.factor(P3_9_2))) + 
...
ggsave(
    path=path, filename= "LIMITE_uso_sino_internet.pdf",
    device="pdf", dpi="retina",
...
ggplot(limite_viv, aes(x = as.factor(ENT), y = FAC_HOG, fill = as.factor(P4_5))) + 
    geom_bar(stat = "identity",position="fill")+
    labs(
...
ggsave(
    path=path, filename= "LIMITE_conexion_internet.pdf",
    device="pdf", dpi="retina",
    width=25, height=14, units="cm"
)


ggplot(limite_viv %>% filter(P4_4==1), aes(x = as.factor(ENT), y = FAC_HOG, fill = as.factor(P4_5))) + 
    geom_bar(stat = "identity",position="fill")+
    labs(
...
ggsave(
    path=path, filename= "LIMITE_conexion_internet_hog_internet.pdf",
    device="pdf", dpi="retina",
    width=25, height=14, units="cm"
)
...
ggplot(limite_res, aes(x = as.factor(ENT), y = FAC_HOGAR, fill = as.factor(P3_9_3))) + 
    geom_bar(stat = "identity",position="fill")+
    labs(
...
ggsave(
    path=path, filename= "LIMITE_uso_sino_cel.pdf",
    device="pdf", dpi="retina",
    width=25, height=14, units="cm"
)
...
ggplot(limite_usu, aes(x = as.factor(ENT), y = FAC_PER, fill = as.factor(P6_1))) + 
    geom_bar(stat = "identity",position="fill")+
    labs(
...
ggsave(
    path=path, filename= "LIMITE_uso_sino_compu.pdf",
    device="pdf", dpi="retina",
    width=25, height=14, units="cm"
)
...
ggplot(limite_usu[limite_usu$P6_1==2,], mapping = aes(y=as.factor(P6_3),x=FAC_PER), )+
    geom_bar(aes(x=after_stat(prop)*100, group=factor(0)), color=lmain,fill=main)+
    labs(
...
ggsave(
    path=path, filename= "LIMITE_razon_no_uso_compu.pdf",
    device="pdf", dpi="retina",
    width=18, height=25, units="cm"
)
...
ggsave(
    path=path, filename= "LIMITE_modo_compu.pdf",
    device="pdf", dpi="retina",
    width=18, height=25, units="cm"
)
...
ggsave(
    path=path, filename= "LIMITE_lugar_compu.pdf",
    device="pdf", dpi="retina",
    width=48, height=27, units="cm"
)
...
ggplot(habilidades_compu, aes(y=Habilidad, x=FAC_PER))+
    geom_bar(stat="identity",linecolor=lmain, fill=main)+
    facet_grid(
        cols = vars(Grupo), rows = vars(ENT), scales = "fixed",
    )+
    labs(
...
ggsave(
    path=path, filename= "LIMITE_habilidad_compu.pdf",
    device="pdf", dpi="retina",
...
ggsave(
    path=path, filename= "LIMITE_uso_compu.pdf",
    device="pdf", dpi="retina",
    width=36, height=20, units="cm"
)

# INTENSIDAD DE USO DE INTERNET
ggplot(limite_usu %>% filter(P7_1==1), aes(P7_4, weight=FAC_PER))+
...
ggsave(
    path=path, filename= "LIMITE_hist_intensidad_internet.pdf",
    device="pdf", dpi="retina",
    width=18, height=25, units="cm"
)
...
ggsave(
    path=path, filename= "LIMITE_equipo_internet.pdf",
    device="pdf", dpi="retina",
    width=18, height=25, units="cm"
)
...
ggsave(
    path=path, filename= "LIMITE_lugar_internet.pdf",
    device="pdf", dpi="retina",
    width=48, height=27, units="cm"
)
...
ggsave(
    path=path, filename= "LIMITE_problema_internet.pdf",
    device="pdf", dpi="retina",
...
ggsave(
    path=path, filename= "LIMITE_plan_cel.pdf",
    device="pdf", dpi="retina",
    width=18, height=25, units="cm"
)
...
ggsave(
    path=path, filename= "LIMITE_uso_cel.pdf",
    device="pdf", dpi="retina",
    width=36, height=20, units="cm"
)
...
ggsave(
    path=path, filename= "LIMITE_uso_internet_capacitacion.pdf",
    device="pdf", dpi="retina",
    width=36, height=20, units="cm"
)
...
ggsave(
    path=path, filename= "LIMITE_uso_internet_comunicacion.pdf",
    device="pdf", dpi="retina",
    width=36, height=20, units="cm"
)
...
ggplot(limite_res%>%filter(P3_10!="NA"), mapping = aes(y=as.factor(P3_10),x=FAC_HOGAR), )+
    geom_bar(aes(x=after_stat(prop)*100, group=factor(0)), color=lmain,fill=main)+
    facet_grid(
        cols = vars(Grupo), rows = vars(ENT), scales = "fixed",
    )+
...
ggsave(
    path=path, filename= "LIMITE_trabajo.pdf",
    device="pdf", dpi="retina",
    width=18, height=25, units="cm"
)
...
ggplot(limite_usu%>%filter(NIVEL!="NA"), mapping = aes(y=as.factor(NIVEL),x=FAC_PER), )+
    geom_bar(aes(x=after_stat(prop)*100, group=factor(0)), color=lmain,fill=main)+
    facet_grid(
        cols = vars(Grupo), rows = vars(ENT), scales = "fixed",
    )+
...
ggsave(
    path=path, filename= "LIMITE_edu.pdf",
    device="pdf", dpi="retina",
    width=18, height=25, units="cm"
)
